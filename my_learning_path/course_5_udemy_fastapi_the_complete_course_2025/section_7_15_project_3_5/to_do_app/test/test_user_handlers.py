...
from . import client
from .helpers import mock_get_db, mock_get_current_user, mock_user
...
def test_get_account_details_handler(mock_user):
...
def test_change_password_handler(mock_user):
    response = client.put(
...
def test_change_password_handler_unauthorized(mock_user):
    response = client.put(
        "user/change-password",
...
def test_change_phone_number_handler(mock_user):
    response = client.put(
