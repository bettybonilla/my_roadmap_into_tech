...
- Write a function called find_pairs that takes in three arguments: arr1, arr2, target and returns a list of all such
...
- The tests for this exercise assume that arr1 is the list being converted to a set
    - Input:
        - Your function should take in the following inputs:
            ...
            - target: an integer
    - Output:
        - Your function should return a list of tuples, where each tuple contains two integers from arr1 and arr2 that
        add up to target
- Example 1:
    arr1 = [1, 2, 3]
    arr2 = [4, 5, 6]
...
def find_pairs(list1: list[int], list2: list[int], target_num: int) -> Optional[list[tuple[int, int]]]:
    set1 = set(list1)
    set2 = set(list2)
    my_list = []
    for i in set1:
