"""
The below shows how we can use decoupling to separate code which makes it
convenient to isolate logic to test or change/update later if needed
"""
...
class Human:
    ...
    # Decoupling is used here to separate code for setting and validating
    # instead of having all the code together which makes it convenient to
    # isolate the validation logic to test or change/update later if needed
    # Public instance method since it is for users to set this attribute which
    # calls the _validate_age private instance method below
    def set_age(self, age: int):
        self._age = self._validate_age(age)

    # Private instance method for internal use only since it contains
    # validation logic
